...
BEGIN
	...
	END CATCH
END
EXECUTE ADDNEWPHONBAN 4,N'CNTT','001','2022-05-09'
GO

-- 3.2 Cập nhật phòng ban có tên CNTT thành phòng IT.
ALTER PROCEDURE UPPB @TENFIND NVARCHAR(15),@MA INT,  @TENREPLACE NVARCHAR(15)
AS
BEGIN
	BEGIN TRY
	IF EXISTS (SELECT TENPHG FROM PHONGBAN WHERE TENPHG = @TENFIND)
	BEGIN
	UPDATE PHONGBAN
	SET TENPHG = @TENREPLACE
	WHERE MAPHG = @MA
	END
...
@MA NVARCHAR(9),@NS DATE, @DCHI NVARCHAR(30), @GTINH NVARCHAR(15),@LUONG FLOAT,@PHONG INT


AS 
BEGIN 
	DECLARE  @MAQL NVARCHAR(9) , @AGE INT
	BEGIN TRY 
		IF EXISTS ( SELECT  MAPHG FROM PHONGBAN WHERE TENPHG = N'IT')
			BEGIN
				...
				ELSE 
					SET @MAQL = N'005'
				SELECT @AGE = DATEDIFF(YEAR,@NS,GETDATE())+1
				...
				(@HO,@LOT,@TEN,@MA,@NS,@DCHI,@GTINH,@LUONG,@MAQL,@PHONG)
				PRINT N'UPDATE SUCCESSFUL!'
			END
		ELSE
			BEGIN
				PRINT N'KHÔNG CÓ PHONG IT'
				RETURN
			END
	END TRY
	BEGIN CATCH
	...
	END CATCH
END
EXECUTE ADDNEWNV N'Nguyễn',N'Văn',N'Hiếu',N'012','1992-01-01',N'Nam',N'001',N'001',3
